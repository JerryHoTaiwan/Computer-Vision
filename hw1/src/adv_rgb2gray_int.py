...
def joint_bilateral_filter(jnt_img, src_img, sigma_color=0.05, sigma_space=1, r_factor=1.5):
    ...
    # (390, 390, 3)
    jnt_img = jnt_img.astype(np.int16)
    src_img = src_img.astype(np.int16)
    radius = np.round(r_factor * sigma_space).astype(np.int16)
    sigma_color *= 255

    jnt_pad = cv2.copyMakeBorder(jnt_img, radius, radius, radius, radius, cv2.BORDER_REFLECT)
    src_pad = cv2.copyMakeBorder(src_img, radius, radius, radius, radius, cv2.BORDER_REFLECT)
    ...
    for y in range(radius, h + radius):
        for x in range(radius, w + radius):
            ...
            if (jnt_patch.shape[2] == 1):
                ...
                hr_b = np.exp(-jnt_diff / (2 * (sigma_color ** 2))).reshape((2*radius+1, 2*radius+1))
                hr_g = np.exp(-jnt_diff / (2 * (sigma_color ** 2))).reshape((2*radius+1, 2*radius+1))
                hr_r = np.exp(-jnt_diff / (2 * (sigma_color ** 2))).reshape((2*radius+1, 2*radius+1))

            elif (jnt_patch.shape[2] == 3):
                jnt_diff_b = np.multiply(jnt_patch[:, :, 0] - jnt_pad[y][x][0], jnt_patch[:, :, 0] - jnt_pad[y][x][0])
                jnt_diff_g = np.multiply(jnt_patch[:, :, 1] - jnt_pad[y][x][1], jnt_patch[:, :, 1] - jnt_pad[y][x][1])
                jnt_diff_r = np.multiply(jnt_patch[:, :, 2] - jnt_pad[y][x][2], jnt_patch[:, :, 2] - jnt_pad[y][x][2])

                hr_b = np.exp(-jnt_diff_b / (2 * (sigma_color ** 2))).reshape((2*radius+1, 2*radius+1))
                hr_g = np.exp(-jnt_diff_g / (2 * (sigma_color ** 2))).reshape((2*radius+1, 2*radius+1))
                hr_r = np.exp(-jnt_diff_r / (2 * (sigma_color ** 2))).reshape((2*radius+1, 2*radius+1))

            response_patch_b = np.multiply(np.multiply(hs, hr_b), src_patch[:, :, 0]) / (np.sum(np.multiply(hs, hr_b)) + 1e-8)
            response_patch_g = np.multiply(np.multiply(hs, hr_g), src_patch[:, :, 1]) / (np.sum(np.multiply(hs, hr_g)) + 1e-8)
            response_patch_r = np.multiply(np.multiply(hs, hr_r), src_patch[:, :, 2]) / (np.sum(np.multiply(hs, hr_r)) + 1e-8)

            tar_img[(y-radius):(y+radius+1), (x-radius):(x+radius+1), 0] += response_patch_b
            tar_img[(y-radius):(y+radius+1), (x-radius):(x+radius+1), 1] += response_patch_g
            tar_img[(y-radius):(y+radius+1), (x-radius):(x+radius+1), 2] += response_patch_r
        ...
        for sc in sigma_color:
            for ss in sigma_space:
                ...
                for weight in candidates:
                    ...
                    if args.use_ans == 1:
                        can_img = can_img.astype(np.int16)
